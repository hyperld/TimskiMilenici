...
const ItemModal: React.FC<ItemModalProps> = ({ itemFormData, setItemFormData, onSave, onClose }) => {
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    const { name, value } = e.target;
    setItemFormData((prev: any) => ({ ...prev, [name]: value }));
  };

  const isProduct = itemFormData?.type === 'product';
    ...
    <div className={styles.modalOverlay} onClick={onClose}>
      <div className={styles.modalContent} style={{ maxWidth: '500px' }} onClick={e => e.stopPropagation()}>
        <header className={styles.modalHeader}>
          <h2>{isEdit ? 'Edit' : 'Add'} {isProduct ? 'Product' : 'Service'}</h2>
        ...
        <form onSubmit={onSave} className={styles.modalForm}>
          ...
          <div className={styles.formActions}>
            ...
            <Button type="submit">Save {isProduct ? 'Product' : 'Service'}</Button>
          </div>
        </form>
      </div>
    </div>
